...
struct Subscription { char userName[50];
char subscriptionType[20]; float cost;
};
...
} else if (strcmp(subs[i].subscriptionType, "Premium") == 0) { premiumCount++;
...
printf("Standard: %d Users, Revenue: %.2f; ", standardCount, standardRevenue); printf("Premium: %d Users, Revenue: %.2f\n", premiumCount, premiumRevenue);
