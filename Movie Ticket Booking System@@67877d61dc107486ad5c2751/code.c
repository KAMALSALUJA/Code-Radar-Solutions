...
struct MovieTicket 
{ 
    char movieName[50]; char ticketType[20]; float price;
};
...
else if (strcmp(tickets[i].ticketType, "Premium") == 0) { premiumTotal += tickets[i].price;
} 
else if (strcmp(tickets[i].ticketType, "VIP") == 0) { vipTotal += tickets[i].price;
}
}
